...
instance : UniversallyOpen (𝔸(n; S) ↘ S) := by
  rw [universallyOpen_iff]
  introv _ h
  rw [← (topologically IsOpenMap).cancel_left_of_respectsIso
    (h.flip.isoIsPullback _ _ (isPullback_map _)).inv,
    h.flip.isoIsPullback_inv_snd _ _ (isPullback_map _)]
  exact isOpenMap_over _
